...
export class CurrentPlaylistStore {
  playlist: Song[] = [];
  ...
  setNextSong = () => {
    ...
    const newActiveurlIndex =
      this.playlist.findIndex((song) => song.audio === this.activeurl) + 1;
    if (this.playlist[newActiveurlIndex])
      this.activeurl = this.playlist[newActiveurlIndex].audio;
    else {
  ...
  addSongNext = (song: Song) => {
    ...
    if (currentSongIndex < 0) {
      ...
      this.playlist.splice(currentSongIndex + 1, 0, song);
    }
  };
